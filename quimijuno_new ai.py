...
class PDFToExcelConverter:
    def __init__(self, root):
        ...
        # Label e Entry para o ficheiro PDF
        ctk.CTkLabel(main_frame, text="Ficheiro PDF de Cotação:", font=('Segoe UI', 12)).pack(pady=5)
        pdf_entry = ctk.CTkEntry(main_frame, textvariable=self.pdf_path, width=400)
        pdf_entry.pack(pady=5)
        ctk.CTkButton(main_frame, text="Procurar", command=self.browse_pdf, font=('Segoe UI', 12)).pack(pady=5)

        # Label e Entry para o ficheiro Excel
        ctk.CTkLabel(main_frame, text="Destino do ficheiro Excel:", font=('Segoe UI', 12)).pack(pady=5)
        excel_entry = ctk.CTkEntry(main_frame, textvariable=self.excel_path, width=400)
        excel_entry.pack(pady=5)
        ctk.CTkButton(main_frame, text="Procurar", command=self.browse_excel, font=('Segoe UI', 12)).pack(pady=5)

        # Barra de progresso
        self.progress = ctk.CTkProgressBar(main_frame, width=400, height=20, mode='determinate')
        self.progress.set(0)
        self.progress.pack(pady=20)

        # Botão de conversão
        ctk.CTkButton(main_frame, text="Converter", command=self.convert, font=('Segoe UI', 12)).pack(pady=10)
    ...
    def convert(self):
        ...
        try:
            self.status_label.configure(text="A processar PDF...")
            self.progress.set(0.33)
            self.root.update_idletasks()

            texto = self.extrair_texto_pdf(self.pdf_path.get())

            self.status_label.configure(text="A extrair dados...")
            self.progress.set(0.66)
            self.root.update_idletasks()

            produtos = self.extrair_dados(texto)

            if produtos:
                self.status_label.configure(text="A criar ficheiro Excel...")
                ...
                self.status_label.configure(text="Conversão concluída com sucesso!")
                messagebox.showinfo("Sucesso", f"Dados exportados com sucesso para {self.excel_path.get()}")
            else:
                self.status_label.configure(text="Nenhum produto encontrado.")
                self.progress.set(0.0)

        except Exception as e:
            self.status_label.configure(text="Erro durante a conversão")
            self.progress.set(0.0)
